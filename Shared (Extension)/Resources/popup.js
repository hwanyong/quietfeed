...
enableToggle.addEventListener('change', async () => {
    ...
    const result = await browser.storage.local.get('profileId')
    const id = result.profileId || null
    if (id) {
        showActiveProfile(id, isEnabled)
    }
...
saveBtn.addEventListener('click', async () => {
    ...
    if (!raw) return
    await browser.storage.local.set({ profileId: raw })
...
// ── Listen for storage changes (e.g. content script auto-detects) ──
